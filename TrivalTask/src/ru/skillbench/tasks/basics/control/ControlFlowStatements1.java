...
/**
 * ЦЕЛЬ ЗАДАЧИ - научиться использовать управляющие конструкции
 * <code>if, case, for, while</code> в Java.<br/>
 * Дополнительно: разобраться с простыми типами, массивами и передачей
 * параметров в методы.<br/>
 * <p/>
 * ЗАДАНИЕ<br/>
 * <dl>
 * <dt>Оператор if</dt>
 ...
 * </code>
 * <p/>
 * Метод main не тестируется, но для себя в main вы можете проверить, что
 * возвращают ваши методы, например: <code>
...
public interface ControlFlowStatements1 {
	/**
	 * Для данного вещественного x найти значение следующей функции f, принимающей
	 * вещественные значения:<br/>
	 * 
	 * <pre>
	 *        | 2*sin(x), если x>0, 
	 *  f(x)= | 
	 ...
	 * соответствующее этому числу:<br/>
	...
	/**
	 * Создать двумерный массив, содержащий 8x5 целочисленных элементов, и присвоить
	 * каждому элементу произведение его индексов: array[i][j] = i*j.
	 * 
	 * @return массив.
	 */
	int[][] initArray();
	 ...
	 * По окончанию каждого года размер вклада увеличивается на P процентов (вклад с
	 * капитализацией процентов).<br/>
	 * По заданному P определить, через сколько лет размер вклада превысит $5000, а
	 * также итоговый размер вклада.
	 * 
	...
	/**
	 * Вспомогательный тип для метода
	 * {@link ControlFlowStatements1#calculateBankDeposit(double)}.<br/>
	 * Примечание: Специальный тип здесь необходим, поскольку метод должен вернуть
	 * ДВА значения, в то время как через аргументы метода значения просто так
	 * вернуть нельзя (передача параметров в методы в Java - только по значению, а
	 * не по ссылке).<br/>
	 * ВОПРОС: как можно было бы обойтись без специального типа, но вернуть два
	 * значения в calculateBankDeposit? (есть несколько способов, возможных с точки
	 * зрения синтаксиса, хотя и все они нехороши с точки зрения ООП)
	 */
